...
def tag_list(page=None):
    ...
    if page is None:
        page = 1
    # 设置per_page每页显示多少个数据
    page_tags = Tag.query.order_by(Tag.id.desc()).paginate(page=page, per_page=1)
...
def collect_list(page=None):
    if page is None:
        page = 1
    page_moviecollects = Moviecol.query.join(
        Movie
    ).join(
        User
    ).filter(
...
def collect_delete(delete_id=None):
    moviecollect = Moviecol.query.get_or_404(delete_id)
    db.session.delete(moviecollect)
    db.session.commit()
    flash('取消收藏成功！', category='ok')
    return redirect(url_for('admin.comment_list', page=1))
...
def auth_list(page=None):
    if not page:
        page = 1
    page_auths = Auth.query.order_by(Auth.addtime.desc()).paginate(page=page, per_page=10)
